...
3. **神速の検証 (Godspeed Verification)**:
   ...
   また、以下の補助パラメータを併用することで、より迅速な検証が可能となる。
   
   - `&debug=true`: 開発者用盤（デバッグパネル）を開く
   - `&mobile=1`: 端末（スマホ）版レイアウトを強制する
   
   これにより、タイトル画面やプロローグの待機（トークンの浪費）を排除し、目的のUIやロジックを一瞬で監査可能となる。

---

## 🛡️ 無限ループの封印 (Eternal Loop Prevention)

ブラウザエージェントによる検証時は、リソース枯渇を防ぐため以下の制約を厳守すること（v4.22.10b）。

1. **神速の撤退 (Fail-fast)**: 
   一つのアクション（クリック等）の後、画面に変化がない場合は最大 3秒 で待機を打ち切り、Console ログを撮影して即座に終了せよ。
2. **手数の制限 (Max Steps)**: 
   一回の検証タスクにつき、最大 15 ステップ 以内に結論を出せ。20ターン以上の検証が必要な場合は、途中で一旦タスクを区切り、状況を報告せよ。
3. **理の直接参照 (Console Audit)**: 
   画面を目視するだけでなく、`window.debugVars` などの内部変数を Console から直接参照し、ステートの遷移を位階（フラグ）レベルで確定させよ。
